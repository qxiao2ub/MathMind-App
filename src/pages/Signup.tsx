...
const yearLevels = ["Year 9", "Year 10"];
...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await signUp(email, password, name, yearLevel);
      toast.success("Welcome to MathMind!");
      navigate("/dashboard");
    } catch (error: any) {
  ...
  return (
    <AuthSplitLayout
      brandTitle="Start learning maths the smart way."
      brandSubtitle="Free to start. Your AI tutor, structured lessons and quizzes are ready to go."
    >
      <SEO title="Sign up | MathMind" description="Create a MathMind account to start learning maths with AI tutoring, practice questions, and curriculum-aligned lessons." />
      <div className="animate-slide-up">
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-sm">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2.5 py-2.5 sm:py-3 rounded-xl border border-border bg-background hover:border-primary/40 hover:bg-muted/40 transition-all min-h-[48px] font-medium text-sm sm:text-base text-foreground"
          >
            {isGoogleLoading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold mb-1.5 text-foreground">Year Level</label>
              <div className="relative">
                <GraduationCap className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-muted-foreground" />
                <select
                  value={yearLevel}
                  onChange={(e) => setYearLevel(e.target.value)}
                  className="w-full pl-10 sm:pl-11 pr-4 py-2.5 sm:py-3 rounded-xl bg-background border border-input focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all appearance-none cursor-pointer text-sm text-foreground"
                  required
                >
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="btn-primary w-full flex items-center justify-center gap-2 min-h-[48px] text-sm sm:text-base font-semibold mt-1"
            >
              {isLoading ? (
